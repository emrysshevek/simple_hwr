...
class CCNN(nn.Module):
    def __init__(self, cnnOutSize=1024, nc=3, leakyRelu=False, type="default", conv_op=nn.Conv2d, verbose=False):
        """ Height must be set to be consistent; width is variable, longer images are fed into BLSTM in longer sequences
    ...
    def default_CNN(self, nc=3, leakyRelu=False):
        ...
        def convRelu(i, batchNormalization=False):
            ...
            nOut = nm[i]
            cnn.add_module('conv{0}'.format(i),
                           self.conv_op(in_channels=nIn, out_channels=nOut, kernel_size=ks[i], stride=ss[i], padding=ps[i]))
            if batchNormalization:
                cnn.add_module('batchnorm{0}'.format(i), nn.BatchNorm2d(nOut))
            if leakyRelu:
                cnn.add_module('relu{0}'.format(i),
